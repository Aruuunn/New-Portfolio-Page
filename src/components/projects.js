...
function MediaCard(props) {
  ...
  const { name, url, repo } = list[idx-1]
    ...
    >
      ...
      >
        <div>
          ...
          <Grid container justify={lg ? "space-between": "space-around"} style={{marginTop:"10px"}}>
            {data.allFile.edges.map((image) => 
             { 
           
             return <MediaCard fluid={image.node.childImageSharp.fluid}  idx={parseInt(image.node.base.split('.')[0][1])}/>
            })}
          </Grid>
        </div>
      </Grid>
